<template>
	<view class="songinfo-container animated fadeInUp faster">
		...
		<view class="action-list">
			<view class="action-item" v-for="(item, index) in actions" :key="index" @click="gotoPage(index)">
				<view class="icon">
		...
		methods: {
			gotoPage(index) {
				switch (index) {
					case 0: {
						uni.navigateTo({
							url: '/pages/artist/artist?artistId=' + this.songInfo.ar[0].id
						})
						break
					}
					case 1: {
						uni.navigateTo({
							url: '/pages/playlist/playlistDetail?playlistId=' + this.songInfo.al.id
						})
						break
					}
				}
